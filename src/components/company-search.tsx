  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-400">Deep-dive any company. Get Tines fit assessment, buying signals, and outreach strategy.</p>
      </div>
          ...
          <div className="space-y-2">
            {STEPS.map((s, i) => (
              <div key={i} className={`flex items-center gap-3 text-sm transition-all duration-300 ${
                i < step ? 'text-emerald-400' : i === step ? 'text-purple-400' : 'text-slate-600'
              }`}>
                {i < step ? (
      ...
      {result && (
        <div className="p-6 rounded-xl bg-white/[0.03] border border-white/10">
          <div className="flex justify-end mb-4">
            <button onClick={copyAll} className="text-xs text-purple-400 hover:text-purple-300 flex items-center gap-1">
      ...
      {!loading && !result && !error && (
        <div className="text-center py-16">
          ...
          <p className="text-slate-400 text-sm">Enter a company name to generate a full Tines intelligence brief</p>
